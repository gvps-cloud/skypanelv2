...
export const normalizeProviderStatus = (
...
export const mapFirewallSummary = (
	entry: any,
	attachment?: any,
): any | null => {
...
export const mapConfigProfile = (entry: any): any | null => {
	...
	const rawDevices =
		entry?.devices && typeof entry.devices === "object" ? entry.devices : {};
	const devices: Record<
		string,
		{ disk_id: number | null; volume_id: number | null }
	> = {};
	for (const slot of ["sda", "sdb", "sdc", "sdd", "sde", "sdf", "sdg", "sdh"]) {
		const raw = rawDevices[slot];
		if (raw && typeof raw === "object") {
			devices[slot] = {
				disk_id: toNumberOrNull(raw.disk_id),
				volume_id: toNumberOrNull(raw.volume_id),
			};
		}
	}
	return {
...
/**
 * Resolve the boot disk ID from a list of mapped config profiles.
 * Looks at the first config's root_device (e.g. "/dev/sda"), extracts the
 * device slot ("sda"), and returns the disk_id from the devices map.
 */
export const resolveBootDiskId = (configs: any[]): number | null => {
	if (!Array.isArray(configs) || configs.length === 0) return null;
	const config = configs[0];
	if (!config) return null;
	const rootDevice: string | null = config.rootDevice ?? null;
