...
router.get("/", async (req: Request, res: Response) => {
	try {
		...
		sendSafeErrorResponse(res, err, 500, {
			fallbackMessage: "Failed to fetch VPS instances",
		});
	}
});
...
router.get("/uptime-summary", async (req: Request, res: Response) => {
	try {
		...
		sendSafeErrorResponse(res, error, 500, {
			fallbackMessage: "Failed to fetch VPS uptime data",
		});
	}
});
...
router.get("/:id", async (req: Request, res: Response) => {
	try {
		...
		sendSafeErrorResponse(res, err, 500, {
			fallbackMessage: "Failed to fetch VPS instance details",
		});
	}
});
	...
	try {
		...
		try {
			...
			res
				...
				.json({ error: structuredError.message, code: structuredError.code });
			return;
		}
		...
		let planIdForInstance: string | undefined;
		...
		try {
			...
			sendSafeErrorResponse(res, walletErr, 500, {
				fallbackMessage: "Failed to verify wallet balance. Please try again.",
			});
			return;
		}
		...
		const configuration = {
			...
			image: resolvedImage,
			template_id:
				isBrandedTemplateId(requestedImage) ||
				isLegacyTemplateId(requestedImage)
					? requestedImage
       ...
       RETURNING *`,
		...
		sendSafeErrorResponse(res, err, 500, {
			fallbackMessage: "Failed to create VPS instance",
		});
	}
});
...
router.post("/:id/boot", async (req: Request, res: Response) => {
	try {
		...
		// Fetch provider display name for whitelabeling
		const providerResult = await query(
			...
			await logActivity(
				{
					...
					entityType: "vps",
					entityId: String(id),
					message: `Booted VPS '${row.label}' on ${providerName}`,
					status: "success",
				},
				req as any,
			);
			...
			await logActivity(
				{
					...
					entityType: "vps",
					entityId: String(id),
					message: `Rebooted VPS '${row.label}' on ${providerName}`,
					status: "success",
				},
				req as any,
			);
...
router.post("/:id/rebuild", async (req: Request, res: Response) => {
	try {
		...
		if (typeof linodeType === "string" && linodeType.trim().length > 0) {
			...
			await logActivity(
				{
					...
					entityType: "vps",
					entityId: String(id),
					message: `Rebuilt VPS '${row.label}' with template '${requestedImage}' on ${providerName}`,
					status: "success",
				},
				req as any,
			);
		...
		res.status(structuredError.statusCode || 500).json({
			error:
				...
				err.message ||
				"Failed to rebuild VPS instance",
			code: structuredError.code,
			details: structuredError.details,
		});
	}
...
router.post("/:id/backups/schedule", async (req: Request, res: Response) => {
	try {
		...
		let dayValue: string | null | undefined;
		if (rawDay !== undefined) {
			if (rawDay === null) {
				dayValue = null;
		...
		let windowValue: string | null | undefined;
...
router.get(
	"/:id/networking/ipv6-rdns-records",
	async (req: Request, res: Response) => {
		try {
			const { id } = req.params;
			const user = (req as any).user;
			...
			try {
				...
				console.warn(
					"Failed to fetch account IPs for IPv6 RDNS records:",
					fetchErr,
				);
			}
...
router.put("/:id/watchdog", async (req: Request, res: Response) => {
	try {
		const { watchdog_enabled } = (req.body || {}) as {
			watchdog_enabled?: unknown;
		};

		if (typeof watchdog_enabled !== "boolean") {
			return res
				.status(400)
				.json({ error: "watchdog_enabled must be a boolean" });
		}
...
router.delete("/:id", async (req: Request, res: Response) => {
	try {
		...
		try {
			...
			return res.status(400).json({
				error: error.message || "Two-factor authentication verification failed",
			});
		}
