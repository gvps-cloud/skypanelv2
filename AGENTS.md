...
Detailed patterns live in `.github/instructions/` — read these for authoritative route-writing, frontend, migration, and testing conventions:
...
- `api/` — Express 4 + TypeScript backend (ESM, `"type": "module"`).
- `lib/` — Shared workspace packages (pnpm workspace; root uses npm). Versions pinned via `pnpm-workspace.yaml` `catalog:` — always `pnpm install` in `lib/`, never `npm install`.
  - `lib/api-client-react` — TanStack Query hooks.
  - `lib/api-spec` — OpenAPI spec + Orval codegen (`pnpm codegen` in that dir).
...
- Three product surfaces: Public Marketing (marketing navbar + `src/styles/home.css`), Customer Portal (dashboard, VPS, billing, tickets, hosting), Admin Dashboard (org/user mgmt, blog CMS, platform controls).
...
- `npm run start`: Production API server. Has `prestart` hook that kills ports 3001/5173/8000 automatically.
...
- `npm run verify:prod`: check + lint + test + security + coverage + scan + audit + env.
- `npm run docs:api:sync`: Sync API docs manifest to `src/lib/apiRouteManifest.ts` (runs automatically via `predev`/`prebuild`/`preclient:dev`).
...
- **PostCSS warning**: `postcss.config.js` says "DON'T EDIT THIS FILE" three times. Leave it alone.
...
- **API key auth globally**: `authenticateApiKey` middleware runs at the `/api` level (`api/app.ts:344`), setting `req.user` for requests with `X-API-Key` or `Bearer sk_live_*` headers, before route-specific auth.
- **Frontend route guards** (from `src/App.tsx`): `<ProtectedRoute>` (auth + sidebar), `<AdminRoute>` (admin), `<StandaloneProtectedRoute>` (auth, no sidebar, for SSH console), `<HostingEnabledRoute>` (feature-flag gated), `<HostingMarketingGate>` (redirects if hosting disabled), `<RegistrationEnabledRoute>` (redirects if registration disabled), `<MaintenanceGuard>` (redirects non-admins to `/maintenance`).
...
- **`STARTUP_SIDE_EFFECTS_ENABLED`**: Setting to `false` disables metrics persistence, billing cron, Bunny CDN refresh, and notification service listeners on startup. Useful for test boots and safe local development.

## Environment

See `repo-docs/ENVIRONMENT_VARIABLES.md` for the full reference. Key required vars:
- `DATABASE_URL`, `JWT_SECRET`, `SSH_CRED_SECRET`
- Encryption rotation via `*_PREVIOUS` env vars
...
- **Append Only**: Never modify an existing migration; add the next numbered file (zero-padded `NNN_*`). Checksum validation warns on changes but continues.
...
- **Key areas**: Blog (`api/routes/blog.ts` + `api/routes/admin/blog.ts`), Tickets (`api/routes/support.ts` + `api/routes/admin/tickets.ts`), Maintenance (`api/routes/admin/platform.ts` + `MaintenanceGuard` in `src/App.tsx`), Theme (dual-path frontend `ThemeContext` + API `api/routes/theme.ts`), Notifications (PG LISTEN/NOTIFY → `NotificationService` → SSE endpoint).
