...
const LazySSHTerminal = lazy(() =>
	import("@/components/VPS/SSHTerminal").then((m) => ({
		default: m.SSHTerminal,
	})),
);
...
const statusStyles: Record<string, string> = {
	...
	stopped: "border border-border bg-muted text-muted-foreground",
	provisioning:
...
const VPSDetail: React.FC = () => {
	...
	const [availableImages, setAvailableImages] = useState<
		Array<{ id: string; label: string; vendor: string }>
	>([]);
	const [imagesLoading, setImagesLoading] = useState<boolean>(false);
	const [rebuildSSHKeys, setRebuildSSHKeys] = useState<string[]>([]);
	const [organizationSSHKeys, setOrganizationSSHKeys] = useState<
	...
	const [rebuildDiskEncryption, setRebuildDiskEncryption] =
		useState<string>("");
	const [rebuildMaintenancePolicy, setRebuildMaintenancePolicy] =
		useState<string>("");
	const [rebuildShowAdvanced, setRebuildShowAdvanced] =
	...
	const configBootDiskId = useMemo(() => {
		const configs = detail?.providerConfigs;
		if (!Array.isArray(configs) || configs.length === 0) return null;
		const config = configs[0];
		if (!config) return null;
		const rootDevice = config.rootDevice ?? null;
		if (!rootDevice) return null;
		const slot = rootDevice.replace("/dev/", "");
		const deviceEntry = config.devices?.[slot];
		if (!deviceEntry) return null;
		return deviceEntry.disk_id ?? null;
	}, [detail?.providerConfigs]);

	const effectiveBootDiskId = useMemo(
		() => detail?.osDiskId ?? configBootDiskId ?? null,
		[detail?.osDiskId, configBootDiskId],
	);
	...
	const loadData = useCallback(
		async (options?: { silent?: boolean }) => {
			...
			try {
				const payload = await apiClient.get<VpsDetailResponse>(
					`/api/vps/${id}`,
				);
	...
	const loadNetworkingConfig = useCallback(async () => {
		...
		try {
			const payload = await apiClient.get<{
				config?: { rdns_base_domain?: string };
			}>("/api/vps/networking/config");
	...
	const performRebuild = useCallback(async () => {
		...
		try {
			await apiClient.post(`/api/vps/${detail.id}/rebuild`, {
				...
				...(rebuildDiskEncryption
					? { diskEncryption: rebuildDiskEncryption }
					: {}),
				...(rebuildMaintenancePolicy
					? { maintenancePolicy: rebuildMaintenancePolicy }
					: {}),
			});
			...
			try {
				...
				const body =
					action === "snapshot"
						...
						: undefined;
				await apiClient.post(`/api/vps/${detail.id}/backups/${endpoint}`, body);
				if (action === "snapshot") {
		...
		try {
			...
			setIpv6RdnsDialog((prev) => ({
				...
				existingRecords: domain.trim()
					? [
							...prev.existingRecords.filter(
								(r) => r.address !== ipAddress.trim(),
							),
		...
		async (enabled: boolean) => {
			...
			try {
				...
				const message =
					err instanceof Error
						? err.message
						: "Failed to update watchdog setting";
				toast.error(message);
			} finally {
	...
	const saveNotes = useCallback(async () => {
		...
		setNotesSaving(true);
		try {
			const payload = await apiClient.put<{ message?: string }>(
				`/api/vps/${detail.id}/notes`,
				{
					notes: notesValue,
				},
			);
					...
					<div className="flex-1 min-w-0 space-y-6 sm:space-y-8">
						...
						{activeTab === "disks" && (
							<VPSDisksTab
								...
								bootDiskId={effectiveBootDiskId}
							/>
						)}
			...
			>
				<DialogContent className="sm:max-w-lg">
					<DialogHeader>
						...
						<DialogDescription>
							This will{" "}
							<strong className="text-destructive">destroy all data</strong> on
							this server and reinstall with a fresh operating system. This
					...
					<div className="space-y-4 py-2">
						...
						<div className="space-y-2">
							...
							{sshKeysLoading ? (
								...
								<div className="rounded-lg border border-dashed p-4 text-center">
									<p className="text-sm text-muted-foreground">
										No SSH keys found. You can add SSH keys in the SSH Keys
										page.
									</p>
								</div>
							) : (
						...
						<div className="border rounded-md">
							...
							>
								Advanced Options
								<ChevronDown
									className={`h-4 w-4 transition-transform ${rebuildShowAdvanced ? "rotate-180" : ""}`}
								/>
							</button>
							{rebuildShowAdvanced && (
								<div className="space-y-4 border-t px-3 py-3">
									...
									<div className="flex items-center justify-between">
										<div>
											...
											<p className="text-xs text-muted-foreground">
												Automatically start the server after rebuild completes
											</p>
										</div>
										<button
											type="button"
									...
									{/* Disk Encryption */}
									<div className="space-y-2">
										<Label htmlFor="rebuild-disk-encryption">
											Disk Encryption
										</Label>
										<Select
											value={rebuildDiskEncryption || "default"}
											onValueChange={(v) =>
												setRebuildDiskEncryption(v === "default" ? "" : v)
											}
										>
											<SelectTrigger
												className="w-full"
												id="rebuild-disk-encryption"
											>
									...
									{/* Maintenance Policy */}
									<div className="space-y-2">
										<Label htmlFor="rebuild-maintenance-policy">
											Maintenance Policy
										</Label>
										<Select
											value={rebuildMaintenancePolicy || "default"}
											onValueChange={(v) =>
												setRebuildMaintenancePolicy(v === "default" ? "" : v)
											}
										>
											<SelectTrigger
												className="w-full"
												id="rebuild-maintenance-policy"
											>
												<SelectValue placeholder="Default" />
											</SelectTrigger>
											<SelectContent>
												<SelectItem value="default">Default</SelectItem>
												<SelectItem value="linode/migrate">
													Live Migration
												</SelectItem>
												<SelectItem value="linode/power_off_on">
													Power Off / On
												</SelectItem>
											</SelectContent>
										</Select>
										<p className="text-xs text-muted-foreground">
											Defines how the server behaves during provider maintenance
										</p>
									</div>
								</div>
							)}
						...
						<div className="space-y-2 rounded-md border border-destructive/50 bg-destructive/5 p-3">
							...
							>
								Type <strong>{detail?.label || "instance-label"}</strong> to
								confirm
							</Label>
							<Input
								id="rebuild-confirm"
				...
				<DialogContent className="w-full max-w-md sm:max-w-lg max-h-[90dvh] flex flex-col overflow-hidden">
					...
					<ScrollArea className="flex-1">
						...
						<div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
							<div className="space-y-1">
								<Label htmlFor="ipv6-rdns-address" className="text-xs">
									IPv6 address
								</Label>
								<Input
							...
							<div className="space-y-1">
								<Label htmlFor="ipv6-rdns-domain" className="text-xs">
									Domain
								</Label>
								<Input
									id="ipv6-rdns-domain"
						...
						<p className="text-[10px] text-muted-foreground -mt-1">
							Leave domain blank to clear rDNS. Records may take up to 24 hours
							to propagate.
						</p>
					...
					<DialogFooter className="pt-1">
						...
						<Button
							...
							disabled={
								ipv6RdnsDialog.saving || !ipv6RdnsDialog.ipAddress.trim()
							}
						>
							{ipv6RdnsDialog.saving ? (
								<>
				...
				<DialogContent className="max-w-[90vw] w-full h-[80vh] flex flex-col p-0 gap-0 bg-background border-border">
					...
					<div className="flex-1 overflow-hidden relative bg-background">
						{sshModalOpen && detail?.id && (
							...
							>
								<LazySSHTerminal instanceId={detail.id} fitContainer={true} />
							</Suspense>
						)}
					</div>
				</DialogContent>
