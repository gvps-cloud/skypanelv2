...
import type { BackupPricing, BackupSummary, VpsInstanceDetail } from "./types";
...
const buildDetail = (
	overrides: Partial<VpsInstanceDetail> = {},
): VpsInstanceDetail => ({
	id: "vps-1",
...
const renderBackupsTab = (
	overrides: Partial<ComponentProps<typeof BackupsTab>> = {},
) => {
...
describe("VPSDetail BackupsTab", () => {
	it("uses readable contrast classes for pricing and manual snapshot callouts", () => {
		const manualSnapshot = buildBackup({
			id: 202,
			label: "Manual pre-upgrade snapshot",
			type: "snapshot",
		});
		const pricing: BackupPricing = {
			monthly: 2.02,
			hourly: 0.002767,
			currency: "USD",
		};
		...
		const manualSnapshotsCard = screen
			.getByText("Manual snapshots")
			.closest("div.rounded-xl");
