...
export interface TransferUsageSummaryItem {
...
export interface TransferUsageSummary {
...
class PaymentService {
  ...
  async getTransferSummary(): Promise<{
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/payments/billing/transfer-summary`, {
      ...
      if (!response.ok) {
        return {
          success: false,
          error: data.error || 'Failed to load transfer summary',
        };
      }
