...
import { deleteIPAddress, listIPs, type IPAMIPAddress, updateReverseDNS } from "@/services/ipamService";
...
export function IPAddressTable() {
  ...
  const [prefixContextFilter, setPrefixContextFilter] = useState<PrefixContextFilter>("all");
  ...
  const deleteMutation = useMutation({
    ...
    onError: (error: Error) => toast.error(error.message || "Failed to delete IP"),
  });

  const allIps = useMemo(() => ipsQuery.data?.data ?? [], [ipsQuery.data?.data]);
  ...
  const filteredIps = useMemo(() => {
    ...
    return allIps.filter((ip) => {
      ...
      if (familyFilter !== "all" && getAddressFamily(ip) !== familyFilter) return false;
      if (scopeFilter !== "all" && (scopeFilter === "public" ? !ip.public : ip.public)) return false;
      if (rdnsFilter !== "all" && (rdnsFilter === "with-rdns" ? !hasRdns(ip) : hasRdns(ip))) return false;
      if (
        prefixContextFilter !== "all" &&
        (prefixContextFilter === "with-prefix" ? !hasPrefixContext(ip) : hasPrefixContext(ip))
      ) {
        return false;
      }
  ...
  }, [allIps, familyFilter, prefixContextFilter, rdnsFilter, scopeFilter, search]);
  ...
  useEffect(() => {
    ...
    if (!selectedAddress || !filteredIps.some((ip) => ip.address === selectedAddress)) {
  ...
  const ipv4Count = filteredIps.filter((ip) => getAddressFamily(ip) === "ipv4").length;
  const ipv6Count = filteredIps.filter((ip) => getAddressFamily(ip) === "ipv6").length;
  const prefixContextCount = filteredIps.filter((ip) => hasPrefixContext(ip)).length;
  const ipsError = ipsQuery.error instanceof Error ? ipsQuery.error.message : null;

  const saveDirectRdns = async (overrideValue?: string) => {
    if (!selectedIp) return;
    setSavingDirectRdns(true);
    const trimmed = (overrideValue ?? directRdns).trim();
    const result = await updateReverseDNS(selectedIp.address, trimmed.length > 0 ? trimmed : null);
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
        <Card>
          ...
          <CardContent>
            ...
            <p className="mt-1 text-xs text-muted-foreground">Allocated addresses</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-3">
          ...
          <CardContent>
            ...
            <p className="mt-1 text-xs text-muted-foreground">{search ? "Matching" : "On this page"}</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-3">
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-sm font-medium">IPv6 Prefix Context</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{prefixContextCount}</div>
            <p className="mt-1 text-xs text-muted-foreground">Rows with prefix metadata</p>
          </CardContent>
        </Card>
      </div>
          ...
          <CardHeader>
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <CardDescription>Data-dense operations view for address inventory and rDNS actions.</CardDescription>
              </div>
              <div className="flex gap-2">
                ...
                >
                  <RefreshCw className={`mr-1 h-4 w-4 ${ipsQuery.isFetching ? "animate-spin" : ""}`} />
                  Refresh
                </Button>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-2 rounded-md border p-2">
              ...
              <FilterChipGroup
                ...
                onChange={(value) => setPrefixContextFilter(value as PrefixContextFilter)}
              />
            </div>
                    ...
                    filteredIps.map((ip) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <TableCell>
                            <Badge variant={family === "ipv4" ? "default" : "secondary"}>{family}</Badge>
                          </TableCell>
                          <TableCell>
                          ...
                          <TableCell className="max-w-[230px] truncate text-sm" title={ip.rdns ?? undefined}>
                            {hasRdns(ip) ? ip.rdns : <span className="text-muted-foreground">Not set</span>}
                          </TableCell>
                          <TableCell>
                            {hasPrefixContext(ip) ? (
                              <Badge variant="secondary">{ip.ipv6Prefixes!.length} prefix</Badge>
                            ) : (
                          ...
                          <TableCell className="text-xs">
                            <div className="space-y-1">
                              <div className="font-mono">{ip.instanceId || "Unassigned"}</div>
                              {ip.vpsLabel ? (
                                <div className="max-w-[180px] truncate text-muted-foreground" title={ip.vpsLabel}>
                                  {ip.vpsLabel}
                                </div>
                              ) : null}
                            </div>
            ...
            {!selectedIp ? (
              ...
              <div className="space-y-4">
                <div className="space-y-2 rounded-md border bg-muted/20 p-3">
                  <div className="font-mono text-sm">{selectedIp.address}</div>
                  <div className="flex flex-wrap gap-2">
                    <Badge variant={getAddressFamily(selectedIp) === "ipv4" ? "default" : "secondary"}>
                      {getAddressFamily(selectedIp)}
                    </Badge>
                    <Badge variant={selectedIp.public ? "default" : "outline"}>
                  ...
                  {selectedIp.vpsLabel ? (
                    <p className="text-xs text-muted-foreground">VPS: {selectedIp.vpsLabel}</p>
                  ) : null}
                </div>
